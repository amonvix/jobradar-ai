...
class JobRadarPipeline:
    ...
    def run(self) -> int:
        ...
        keyword_config = load_keywords(self.keywords_path)
        cv_text = load_cv(self.cv_path)

        collector = WebCollector(debug=self.debug, debug_html_dir=self.debug_html_dir)
        matcher = ResumeMatcher(cv_text=cv_text, keyword_config=keyword_config)
        ranker = JobRanker(matcher=matcher)

        all_jobs = []
        for site in sites:
            all_jobs.extend(collector.collect_from_site(site=site, keywords=keyword_config.positive_keywords))
